...
public class PostPedidoGranelTask extends AsyncTask<String, Void, Boolean> {
	
	private PostResultListener listener;
	//private List<EnviarPedido> hc = null;
	private List<EnviarPedidoGranel> pedido;
	
	public PostPedidoGranelTask(PostResultListener listener, List<EnviarPedidoGranel> pedido) {
	...
	protected Boolean doInBackground(String... params) {
		...
		while(i < Constants.LOOP_TASK){
			...
			Log.e(Constants.TAG, "PostPedidoTask >>>>>>> Mensagem ERRO inicio "+Statics.mensagemErro);
					...
					try {
						hr = UtilWS.postSalvaPedidoGranel(pedido);
