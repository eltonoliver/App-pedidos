...
public class HistoricoPedidoItemTask extends AsyncTask<String, Void, List<HistoricoPedidoItens>> {
	
	public interface HistoricoPedidoItemResultListener{
		public void HistoricoPedidoItemResult(List<HistoricoPedidoItens> result);
	}
	
	private HistoricoPedidoItemResultListener listener;
	private List<HistoricoPedidoItens> hc = null;
	
	public HistoricoPedidoItemTask(HistoricoPedidoItemResultListener listener) {
	...
	protected List<HistoricoPedidoItens> doInBackground(String... params) {
		...
		while(i < Constants.LOOP_TASK){
			...
			Log.i(Constants.TAG,"Historico doInBackground "+i);
					try {
						hr = UtilWS.getListaHistoricoPedidoItem(pedido);
						Log.i(Constants.TAG,"Historico doInBackground hr"+hr.getStatusLine().getStatusCode());
						
						if(hr.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
							hc = HistoricoPedidoItemParseJSON.parseDados(hr);
	...
	protected void onPostExecute(List<HistoricoPedidoItens> result) {
		listener.HistoricoPedidoItemResult(result);
