...
public class HistoricoPedidoPJTask extends AsyncTask<String, Void, List<HistoricoPedido>> {
	
	public interface HistoricoPedidoPJResultListener{
		public void HistoricoPedidoPJResult(List<HistoricoPedido> result);
	}
	
	private HistoricoPedidoPJResultListener listener;
	private List<HistoricoPedido> hc = null;
	
	public HistoricoPedidoPJTask(HistoricoPedidoPJResultListener listener) {
	...
	protected List<HistoricoPedido> doInBackground(String... params) {
		...
		while(i < Constants.LOOP_TASK){
			...
			Log.i(Constants.TAG,"Historico doInBackground "+i);
					try {
						hr = UtilWS.getListaHistoricoPedidoPJ(codConsumidor, opcao);
						Log.i(Constants.TAG,"Historico doInBackground hr"+hr.getStatusLine().getStatusCode());
						
						if(hr.getStatusLine().getStatusCode() == HttpStatus.SC_OK){
							hc = HistoricoPedidoPJParseJSON.parseDados(hr);
	...
	protected void onPostExecute(List<HistoricoPedido> result) {
		listener.HistoricoPedidoPJResult(result);
